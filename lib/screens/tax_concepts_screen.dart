  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<Concepto>>(
        future: DatabaseService().conceptoDao
            .getBySubCategoriaId(subCategoriaId, langCode: currentLang),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (conceptos.isEmpty) {
              return const Center(
                  child: Text(
                      'Aucune taxe trouvée pour cette sous-catégorie.')); // Localization later
            }
            ...
            return const Center(
                child: Text(
                    'Aucune taxe disponible pour cette sous-catégorie.')); // Localization later
          }
        },
      ),
    );

...
class DatabaseService {
  ...
  Future<void> initialize(
    ...
    try {
      ...
      _db = await DatabaseMigration.openDatabaseWithMigration(
      ...
      if (seedData) {
        ...
        if (ministerioCount == 0 || forceReset) {
          developer.log('Seeding database with initial data...',
              name: 'DatabaseService');
  ...
  Future<String> exportToJson({String? path, String? langCode}) async {
    try {
      ...
      final jsonString =
          const JsonEncoder.withIndent('  ').convert(exportData['ministerios']);
      ...
      if (path != null) {
        ...
        filePath = join(dir.path,
            'taxasge_export_${DateTime.now().millisecondsSinceEpoch}.json');
      }

      final file = File(filePath);
      await file.writeAsString(jsonString);

      developer.log('Database exported to JSON: $filePath',
          name: 'DatabaseService');
      return filePath;
    } catch (e) {
      developer.log('Error exporting database to JSON: $e',
          name: 'DatabaseService');
  ...
  Future<Map<String, dynamic>?> getConceptoWithDetails(String conceptoId,
    ...
    try {
      ...
      developer.log('Error getting concepto with details: $e',
          name: 'DatabaseService');

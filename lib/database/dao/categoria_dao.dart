...
class CategoriaDao {
  ...
  ///
  /// Les catégories sont optionnellement triées par le champ spécifié et dans la langue spécifiée.
  Future<List<Categoria>> getBySectorId(String sectorId, {String? orderBy, String? langCode}) async {
    try {
      // Si une langue est spécifiée et aucun ordre n'est défini, trier par le nom dans cette langue
      String? effectiveOrderBy = orderBy;
      if (orderBy == null && langCode != null && _supportedLanguages.contains(langCode)) {
  ...
  Future<int> updateTranslation(String id, String langCode, String nombre) async {
    try {
      ...
      developer.log('Error updating categoria translation: $e', name: 'CategoriaDao');
  ...
  Future<List<Categoria>> searchByName(String searchTerm, {String? langCode}) async {
    try {
      String whereClause;
  ...
  Future<List<Categoria>> getCategoriasWithTranslation(String langCode) async {
    try {
      ...
      developer.log('Error getting categorias with translation: $e', name: 'CategoriaDao');
  ...
  Future<List<Categoria>> getCategoriasWithoutTranslation(String langCode) async {
    try {
      if (!_supportedLanguages.contains(langCode)) {
        throw ArgumentError('Langue non supportée: $langCode');
      }
      ...
      developer.log('Error getting categorias without translation: $e', name: 'CategoriaDao');
  ...
  Future<List<String>> getAvailableLanguagesForCategoria(String categoriaId) async {
    try {
      ...
      developer.log('Error getting available languages for categoria: $e', name: 'CategoriaDao');
